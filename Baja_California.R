...
ConcBaja_California<-Conc%>% 
  filter(entidad=="2")

remove(Conc)

#apparently this is a "flag", IDK what is this shit yet
ConcBaja_California$Nhog <- 1

#DECILES 

#Attaching the data frame
attach(ConcBaja_California) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcBaja_California<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcBaja_California) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcBaja_California$factor,to.data.frame=TRUE)
...
ConcBaja_California$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcBaja_California$MAXT<-ConcBaja_California$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcBaja_California<-ConcBaja_California[with(ConcBaja_California, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcBaja_California$ACUMULA<-cumsum(ConcBaja_California$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcBaja_California[dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcBaja_California<-rbind(ConcBaja_California[1:(dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1]+1),],
                            ConcBaja_California[(dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1]+1):dim(ConcBaja_California[1])[1],])
  
  b1<-tam_dec*i-ConcBaja_California[dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcBaja_California[(dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcBaja_California[(dim(ConcBaja_California[ConcBaja_California$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcBaja_California$ACUMULA2<-cumsum(ConcBaja_California$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcBaja_California$DECIL<-0
...
ConcBaja_California[(ConcBaja_California$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcBaja_California[((ConcBaja_California$ACUMULA2>tam_dec*i)&(ConcBaja_California$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcBaja_California[ConcBaja_California$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Baja_California")
write.dbf(ConcBaja_California,file="ConcBaja_California2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Baja_California")
write.dbf(ConcBaja_California,file="ConcBaja_California2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Baja_California")
Conc2010<-read.dbf("ConcBaja_California2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Baja_California por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Baja_California INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Baja_California NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Baja_California")
Conc2018<-read.dbf("ConcBaja_California2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Baja_California Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Baja_California INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Baja_California NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Baja_California NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Baja_California NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Baja_California NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Baja_California NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Baja_California por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Baja_California Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Baja_California NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Baja_California NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Baja_California INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Baja_California INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
